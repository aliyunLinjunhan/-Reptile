...
# import re
#
# pattern_redian = re.compile("'热点': '(.*?)'")
# pattren_readingamount = re.compile("'阅读量': '(.*?)'")
# pattren_time = re.compile("'时间': '(.*?)'")
# pattren_content = re.compile("'热点导语': '*\[*'(.*?)'\]*")
#
# a = set()
# for line in open(r'C:\Users\Administrator\PycharmProjects\newWork\paqu_gushiwen\redian2(4.12wan).txt', 'r', encoding='UTF-8'):
#     redian = pattern_redian.findall(line)[0]
#     a.add(redian)
# print(len(a))
# for line in open(r'C:\Users\Administrator\PycharmProjects\newWork\paqu_gushiwen\redian2（4.12）.txt', 'r', encoding='UTF-8'):
#     redian = pattern_redian.findall(line)[0]
#     a.add(redian)
# print(len(a))
# print(len(a))
...
#
# for line in open(r'C:\Users\Administrator\PycharmProjects\newWork\paqu_gushiwen\redian(4-11）.txt', 'r', encoding='UTF-8'):
#     print(line)
#     redian = pattern_redian.findall(line)[0]
#     readingamount = pattren_readingamount.findall(line)[0]
#     time = pattren_time.findall(line)[0]
#     content = pattren_content.findall(line)[0]
#     print(redian, readingamount, time, content)
#     # 执行sql语句
#     try:
#         with connection.cursor() as cursor:
#             # 执行sql语句，插入记录
#             sql = "INSERT INTO redian (title, time_readAmount, content, key_words) VALUES (%s, %s, %s, %s)"
#             cursor.execute(sql, (redian, time + ' ' + readingamount, content, ''))
...
pattern_title = re.compile('诗文名称: ([\u4e00-\u9fa5]+)点赞量')
pattren_good = re.compile('点赞量: ([0-9]+)')
pattren_content = re.compile('诗文内容: (.*?。),')
...
for str in open(r'C:\Users\Administrator\PycharmProjects\newWork\paqu_gushiwen\siwen_message.txt', 'r', encoding='UTF-8'):
